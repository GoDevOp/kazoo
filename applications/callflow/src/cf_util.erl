...
-spec mailbox(ne_binary(), ne_binary()) -> {'ok', ne_binaries()} |
                                           {'error', any()}.
...
-spec get_mailbox(ne_binary(), ne_binary()) -> {'ok', ne_binaries()} |
                                               {'error', any()}.
...
-define(VM_COUNT(A,B), lists:sum([1 || M <- A, wh_json:get_value(<<"folder">>, M) =:= B])).

-spec vm_count(wh_json:object()) -> {non_neg_integer(), non_neg_integer()}.
vm_count(JObj) ->
    Messages = wh_json:get_value(<<"messages">>, JObj, []),
    {?VM_COUNT(Messages, <<"new">>), ?VM_COUNT(Messages, <<"saved">>)}.
